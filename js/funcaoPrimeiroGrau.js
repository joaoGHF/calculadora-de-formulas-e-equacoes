var btnFuncPrimeiroGrau = document.getElementById("funcao-primeiro-grau");

btnFuncPrimeiroGrau.addEventListener("click", function(){
    funcaoPrimeiroGrau();
})

function funcaoPrimeiroGrau(){  
    ...
    var label1 = 
    criarLabel("input1", "label-input1", "Insira o valor de 'x':");
    var input1 = criarInput("input1", "input1");

    var label2 = criarLabel("input2", "label-input2", "Insira o valor de 'a':");
    var input2 = criarInput("input2", "input2");

    var label3 = criarLabel("input3", "label-input3", "Insira o valor de 'b':");
    ...
    var textoFormula = criarTextoFormula("f(x) = ax + b");
    ...
    botaoCalcular.addEventListener("click", function (event){
        ...
        if (input1.value.length == 0 || isNaN(input1.value)){
            var p1 = criarParagraphDeErro("O campo 'x' é invalido");
            ulErros.appendChild(p1);
            naoTemErros = false;
        }

        if (input2.value.length == 0 || isNaN(input2.value)){
            var p2 = criarParagraphDeErro("O campo 'a' é invalido");
            ulErros.appendChild(p2);
            naoTemErros = false;
        }

        if (input3.value.length == 0 || isNaN(input3.value)){
            var p3 = criarParagraphDeErro("O campo 'b' é invalido");
        ...
        if (naoTemErros) {
            ...
            var a = parseFloat(input2.value)
            var b = parseFloat(input3.value);

            var resultado = (a * x) + b

            var textoResposta = "f(" + x + ") = " + resultado;
